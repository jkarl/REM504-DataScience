...
For this tutorial we'll use a new Google Earth Engine based web app for aquiring Landsat composit imagery (i.e., LS 5-8).
...
```{r}
setwd("C:/Users/jakal/OneDrive - University of Idaho/Documents/GitHub/REM504-DataScience/Lecture_Slides/timeseries_analysis")
```
...
## Load dataset of Landsat NDVI images for IBPE site of Jornada LTER
load("~/GitHub/REM504-DataScience/Lecture_Slides/timeseries_analysis/DataClass_TimeSeries.RData")
...
```
Landsat NDVI data were downloaded at GeoTIFF images, read into R, and added to a raster brick/stack. Both NDVI values and the Landsat Quality Assessment (QA) values were imported.
...
Plot the first 12 NDVI images from the raster brick
```{r}
...
##LS QA
The QA band from a landsat or MODIS scene contains information on the quality of each pixel. If there are clouds, shadows, snow, haze, etc. It is denoted in the QA band. This is useful for automatically excluding poor image pixels.
...
Create quarterly NDVI averages
```{r}
...
```{r}
## points is a dataset of point locations where field sampling is done
...
```{r}
## Let's save the originals for comparison
p1.orig <- p1
p5.orig <- p5
...
g <- autoplot.zoo(p1.orig)

# Overlay napprox with the original time series
g + geom_line(data=p1.napprox,aes(x=Index,y=p1.napprox),color="blue") +
    geom_line(data=p1,aes(x=Index,y=p1),color="red")+
    ggtitle("napprox with original time series")

# Overlay linear
g + geom_line(data=p1.linear,aes(x=Index,y=p1.linear),color="blue") +
    geom_line(data=p1,aes(x=Index,y=p1),color="red")+
    ggtitle("linear interp w/ original time series")

# Overlay linear
g + geom_line(data=p1.stine,aes(x=Index,y=p1.stine),color="blue") +
    geom_line(data=p1,aes(x=Index,y=p1),color="red")+
    ggtitle("Stine interp w/ original time series")
